...
public class AgendaRecadosDao {
    ...
    public AgendaRecados incluirAgendaRecado(AgendaRecados objAgendaRec) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO AGENDA_RECADOS (DataLanc,Horario,StatusAgenda,NomeUsuarioLogado,IdUsuario,Recados,MensagemEnviada,MensagemRecebida) VALUES(?,?,?,?,?,?,?,?)");
    ...
    public AgendaRecados alterarAgendaRecado(AgendaRecados objAgendaRec) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE AGENDA_RECADOS SET DataLanc=?,Horario=?,StatusAgenda=?,NomeUsuarioLogado=?,IdUsuario=?,Recados=?,MensagemEnviada=?,MensagemRecebida=? WHERE IdLanc='" + objAgendaRec.getIdLanc() + "'");
            pst.setTimestamp(1, new java.sql.Timestamp(objAgendaRec.getDataLanc().getTime()));
            pst.setString(2, objAgendaRec.getHorario());
            pst.setString(3, objAgendaRec.getStatusAgena());
            pst.setString(4, objAgendaRec.getNomeUsuarioLogado());
     ...
     public AgendaRecados alterarStatusAgendaRecado(AgendaRecados objAgendaRec) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE AGENDA_RECADOS SET StatusAgenda=?,MensagemRecebida=? WHERE IdLanc='" + objAgendaRec.getIdLanc() + "'");            
    ...
    public AgendaRecados excluirAgendaRecado(AgendaRecados objAgendaRec) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM AGENDA_RECADOS WHERE IdLanc='" + objAgendaRec.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
